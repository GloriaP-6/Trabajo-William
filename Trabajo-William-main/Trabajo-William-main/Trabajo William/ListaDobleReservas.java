...
public class ListaDobleReservas {
    ...
    public boolean insertar(Reserva var1) {
  ...
  public List<Reserva> obtenerTopNCostos(int n, boolean ascendente) {
    ...
    public void mostrarEstadisticasCompletas() {
        ...
        // Devuelve la lista de reservas como String
    public String imprimirComoString() {
    ...
    // parte del diseño grafico es mi pasion

    // Devuelve las estadísticas como String
    public String estadisticasComoString() {
        StringBuilder sb = new StringBuilder();
        sb.append("=== ESTADÍSTICAS DE RESERVAS ===\n");
        sb.append("Total de reservas: ").append(this.contarReservas()).append("\n");
        sb.append(String.format("Costo promedio: $%.2f\n", this.calcularPromedio()));
        sb.append(String.format("Costo mínimo: $%.2f\n", this.encontrarMinimo()));
        sb.append(String.format("Costo máximo: $%.2f\n", this.encontrarMaximo()));
        sb.append(String.format("Rango: $%.2f\n", this.calcularRango()));
        sb.append(String.format("Mediana: $%.2f\n", this.calcularMediana()));

        Map<Double, Integer> moda = this.calcularModa();
        sb.append("Moda: ");
        if (moda.isEmpty()) {
            sb.append("No hay datos\n");
        } else {
            for (Map.Entry<Double, Integer> entry : moda.entrySet()) {
                sb.append(String.format("$%.2f (aparece %d veces) ", entry.getKey(), entry.getValue()));
            }
            sb.append("\n");
        }

        sb.append(String.format("Varianza: %.2f\n", this.calcularVarianza()));
        sb.append(String.format("Desviación estándar: %.2f\n", this.calcularDesviacionEstandar()));

        // Tabla de frecuencias
        sb.append("\n--- Tabla de Frecuencias ---\n");
        Map<Double, Integer> tabla = this.generarTablaFrecuencias();
        for (Map.Entry<Double, Integer> entry : tabla.entrySet()) {
            sb.append(String.format("$%.2f : %d\n", entry.getKey(), entry.getValue()));
        }

        // Top 3 costos más altos y bajos (puedes cambiar el número si quieres)
        sb.append("\nTop 3 costos más altos:\n");
        List<Reserva> topAltos = this.obtenerTopNCostos(3, false);
        for (Reserva r : topAltos) {
            sb.append(String.format("$%.2f - %s\n", r.getCosto(), r.getCliente()));
        }

        sb.append("\nTop 3 costos más bajos:\n");
        List<Reserva> topBajos = this.obtenerTopNCostos(3, true);
        for (Reserva r : topBajos) {
            sb.append(String.format("$%.2f - %s\n", r.getCosto(), r.getCliente()));
        }
    ...
    // Devuelve la lista de reservas como List<String>
    public List<String> obtenerReservasComoLista() {
        List<String> reservas = new ArrayList<>();
        Nodo actual = cabeza;
        if (actual == null) {
            reservas.add("No hay reservas registradas.");
        } else {
            while (actual != null) {
                reservas.add(actual.getDato().toString());
                actual = actual.getSiguiente();
            }
        }
        return reservas;
